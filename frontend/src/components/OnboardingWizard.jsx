...
const TABLE_TEMPLATES = {
  clientes: {
    ...
    headers: [
      ...
      { key: "email", label: "Email", type: "email", emoji: "✉️" },
      { key: "notas", label: "Notas", type: "text", emoji: "📝" },
      { key: "fechaRegistro", label: "Fecha Registro", type: "date", defaultValue: "today", emoji: "📅" },
    ],
  ...
  reservas: {
    ...
    headers: [
      ...
      { key: "personas", label: "Personas", type: "number", defaultValue: "2", emoji: "👥" },
      { key: "estado", label: "Estado", type: "select", options: ["Pendiente", "Confirmada", "Cancelada"], defaultValue: "Pendiente", emoji: "📊" },
      { key: "notas", label: "Notas", type: "text", emoji: "📝" },
    ],
    permissions: { allowQuery: true, allowCreate: true, allowUpdate: true, allowDelete: false },
  },
  citas: {
    ...
    headers: [
      ...
      { key: "estado", label: "Estado", type: "select", options: ["Programada", "Confirmada", "Cancelada", "Completada"], defaultValue: "Programada", emoji: "📊" },
    ],
    permissions: { allowQuery: true, allowCreate: true, allowUpdate: true, allowDelete: false },
  },
  servicios: {
...
const BUSINESS_TYPES = [
  { id: "salon", icon: "💇", name: "Salón / Barbería / Spa", description: "Citas, clientes, servicios", color: "#ec4899", workspace: { name: "Mi Salón", color: "#ec4899" }, suggestedTables: ["clientes", "citas", "servicios"], agentName: "Asistente de Citas", agentDescription: "Soy el asistente virtual del salón. Puedo agendar citas, consultar disponibilidad y responder sobre servicios y precios." },
  { id: "restaurant", icon: "🍽️", name: "Restaurante / Cafetería", description: "Reservas, pedidos, menú", color: "#f59e0b", workspace: { name: "Mi Restaurante", color: "#f59e0b" }, suggestedTables: ["clientes", "reservas", "pedidos"], agentName: "Asistente de Reservas", agentDescription: "Soy el asistente del restaurante. Gestiono reservas, tomo pedidos y respondo sobre el menú y horarios." },
  { id: "clinic", icon: "🏥", name: "Consultorio / Clínica", description: "Pacientes, citas, historial", color: "#10b981", workspace: { name: "Mi Consultorio", color: "#10b981" }, suggestedTables: ["clientes", "citas", "servicios"], agentName: "Asistente Médico", agentDescription: "Soy el asistente del consultorio. Agendo citas, consulto disponibilidad y oriento a los pacientes." },
  { id: "gym", icon: "🏋️", name: "Gimnasio / Fitness", description: "Miembros, clases, horarios", color: "#ef4444", workspace: { name: "Mi Gimnasio", color: "#ef4444" }, suggestedTables: ["clientes", "servicios", "reservas"], agentName: "Asistente Fitness", agentDescription: "Soy el asistente del gimnasio. Informo sobre clases, horarios y gestiono inscripciones." },
  { id: "store", icon: "🛍️", name: "Tienda / E-commerce", description: "Productos, pedidos, inventario", color: "#3b82f6", workspace: { name: "Mi Tienda", color: "#3b82f6" }, suggestedTables: ["productos", "pedidos", "clientes"], agentName: "Asistente de Ventas", agentDescription: "Soy el asistente de la tienda. Consulto productos, precios, stock y ayudo a procesar pedidos." },
  { id: "education", icon: "🎓", name: "Academia / Educación", description: "Alumnos, clases, pagos", color: "#8b5cf6", workspace: { name: "Mi Academia", color: "#8b5cf6" }, suggestedTables: ["clientes", "servicios", "reservas"], agentName: "Asistente Académico", agentDescription: "Soy el asistente de la academia. Informo sobre cursos, horarios e inscripciones." },
  { id: "services", icon: "🔧", name: "Servicios / Profesional", description: "Clientes, proyectos, cotizaciones", color: "#06b6d4", workspace: { name: "Mis Servicios", color: "#06b6d4" }, suggestedTables: ["clientes", "servicios", "pedidos"], agentName: "Asistente de Servicio", agentDescription: "Soy el asistente de atención al cliente. Oriento sobre servicios, precios y coordino citas." },
  { id: "custom", icon: "✨", name: "Otro tipo de negocio", description: "Configura a tu medida", color: "#6b7280", workspace: { name: "Mi Negocio", color: "#6b7280" }, suggestedTables: ["clientes"], agentName: "Asistente Virtual", agentDescription: "Soy tu asistente virtual. Atiendo clientes, respondo preguntas y gestiono información." },
];

const BOT_PERSONALITIES = [
  { id: "amigable", icon: "😊", name: "Amigable", description: "Cálido, cercano y empático", preview: "¡Hola! 😊 ¿En qué puedo ayudarte hoy?", style: "Usa un tono amigable y cercano. Tutea al cliente, usa emojis ocasionalmente y muéstrate siempre dispuesto a ayudar." },
  { id: "profesional", icon: "💼", name: "Profesional", description: "Formal, preciso y confiable", preview: "Buenos días. ¿En qué puedo asistirle?", style: "Usa un tono profesional y formal. Responde con precisión y cortesía, evita los emojis excesivos." },
  { id: "experto", icon: "🧠", name: "Experto", description: "Conocedor, detallado y técnico", preview: "Estoy listo para atenderte. ¿Qué necesitas?", style: "Presenta el conocimiento con autoridad. Ofrece detalles relevantes y muéstrate como experto en el área." },
];
  ...
  return (
    <div className="p-8">
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 text-2xl mb-4 shadow-lg shadow-violet-900/40">⚡</div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {BUSINESS_TYPES.map((biz) => (
          <button key={biz.id} onClick={() => onSelect(biz)}
            ...
            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shadow-md transition-transform group-hover:scale-110" style={{ background: biz.color + "22" }}>{biz.icon}</div>
            <div>
...
function StepBot({ botName, setBotName, personality, setPersonality, onBack, onCreate }) {
  ...
  return (
    <div className="p-8">
      <button onClick={onBack} className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 mb-6 transition-colors">← Volver</button>
      <div className="mb-6">
        <h2 className="text-xl font-bold text-white mb-1">🤖 Tu asistente de IA</h2>
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4 mb-6">
        ...
        <div className="flex items-start gap-3">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-indigo-500 flex items-center justify-center text-sm flex-shrink-0">🤖</div>
      ...
      <button onClick={onCreate} disabled={!botName.trim()}
        className="w-full py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-semibold hover:from-violet-500 hover:to-indigo-500 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-violet-900/30 text-sm">
        ✨ Crear mi negocio
      </button>
    </div>
  );
      ...
      {error ? (
        ...
        <>
          <div className="relative inline-flex items-center justify-center mb-6">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-4xl shadow-2xl shadow-violet-900/50">
              {progress < 40 ? "🏗️" : progress < 70 ? "📊" : progress < 90 ? "🤖" : "✨"}
            </div>
...
function StepDone({ summary, onComplete }) {
  ...
  return (
    <div className={`p-8 text-center transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
      <div className="text-5xl mb-4" style={{ filter: "drop-shadow(0 0 16px #7c3aed88)" }}>🎉</div>
      ...
      <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5 mb-6 text-left space-y-3">
        <SummaryItem icon="🏢" label="Proyecto creado" value={summary.workspaceName} />
        {summary.tables.length > 0 && <SummaryItem icon="📊" label={`${summary.tables.length} tabla${summary.tables.length > 1 ? "s" : ""} creada${summary.tables.length > 1 ? "s" : ""}`} value={summary.tables.join(", ")} />}
        <SummaryItem icon="🤖" label="Asistente IA" value={summary.agentName} highlight />
      </div>
