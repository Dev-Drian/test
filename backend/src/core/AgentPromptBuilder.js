...
Mantén respuestas concisas (2-3 oraciones cuando sea posible).`;
  ...
  _buildCapabilities(capabilities, tablesInfo) {
    ...
    if (tablesInfo.length > 0) {
      ...
      lines.push('');
      lines.push('   🎯 REGLA DE INTERPRETACIÓN SEMÁNTICA:');
      lines.push('   Cuando el usuario pregunta "qué [X] tienen/hay", busca una tabla que coincida:');
      lines.push('   • "qué destinos tienen" → query_records de tabla Destinos');
      lines.push('   • "qué servicios hay disponibles" → query_records de tabla Servicios');
      lines.push('   • "muéstrame los productos" → query_records de tabla Productos');
      lines.push('   • "qué opciones hay" → analiza el contexto y consulta la tabla relevante');
      lines.push('   ⚠️ NO confundas "disponibles" o "hay" con un nombre de tabla literal');
      lines.push('   El usuario está pidiendo consultar datos, no una tabla llamada "disponible"');
      lines.push('');
